...
def main():
    """Process all matching mesh pairs between obj_4ddress_files and obj_pifuhd_files directories."""
    import os
    import glob
    ...
    print(f"Aligned meshes will be saved to: {aligned_dir}")
    
    # Get all files in the obj_4ddress_files directory
    address_files = {os.path.basename(f): f for f in glob.glob(os.path.join(address_dir, "*.obj"))}
    
    # Get all files in the obj_pifuhd_files directory
    pifuhd_files = {os.path.basename(f): f for f in glob.glob(os.path.join(pifuhd_dir, "*.obj"))}
    
    # Find common filenames
    common_files = set(address_files.keys()) & set(pifuhd_files.keys())
    print(f"Found {len(common_files)} matching mesh pairs to process")
    
    # Process each pair
    for i, filename in enumerate(sorted(common_files)):
        print(f"\nProcessing pair {i+1}/{len(common_files)}: {filename}")
        
        # Get the full paths
        mesh1_path = address_files[filename]  # 4ddress file (target)
        mesh2_path = pifuhd_files[filename]   # pifuhd file (source to be aligned)
        
        # Define output path for aligned mesh
        aligned_mesh_path = os.path.join(aligned_dir, filename)
        
        try:
            # Align the meshes
            aligned_mesh = align_human_meshes_with_icp(mesh1_path, mesh2_path, decimate=True)
            
            # Save the aligned mesh to the new directory
            aligned_mesh.export(aligned_mesh_path)
            print(f"Successfully aligned and saved to: {aligned_mesh_path}")
